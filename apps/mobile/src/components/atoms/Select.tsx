        ...
        >
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Typography variant="h3">{label || "Choose an Option"}</Typography>
            </View>
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <TouchableOpacity
                  style={[styles.optionItem, item.value === value && styles.optionItemActive]}
                  onPress={() => handleSelect(item.value)}
                >
                  {item.color && <View style={[styles.dot, { backgroundColor: item.color }]} />}
                  <Typography style={item.value === value ? styles.optionTextActive : {}}>
                    {item.label}
                  </Typography>
                  {item.value === value && (
