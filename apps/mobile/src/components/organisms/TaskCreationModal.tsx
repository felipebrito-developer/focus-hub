...
import type { AnyType, NewTask, Task } from "@focus-hub/shared";
...
export function TaskCreationModal({
  ...
  useEffect(() => {
    if (editingTask) {
      ...
      setSelectedTagIds(editingTask.tags?.map((t: AnyType) => t.id) || []);
    } else {
      setTitle("");
      setDesc("");
      setPriority("medium");
    ...
    <Modal visible={visible} animationType="slide" transparent>
      <View style={styles.modalOverlay}>
        <Card style={styles.modalCard}>
          <Typography variant="h3">{editingTask ? "Edit Task" : "New Task"}</Typography>
